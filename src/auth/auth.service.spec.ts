...
describe('AuthService 測試', () => {
  ...
  it('註冊時 email 格式錯誤', async () => {
    await expect(
      authService.register('bademail', '12345678', 'Sam', 20),
    ).rejects.toThrow('請輸入有效的 Email');
  });

  it('註冊時 密碼長度錯誤', async () => {
    await expect(
      authService.register('test@example.com', '123', 'Sam', 20),
    ).rejects.toThrow('密碼長度需介於 8~60 字元');
  });

  it('註冊時 email 已存在', async () => {
    (userRepo.findOne as jest.Mock).mockResolvedValue({} as User);
    await expect(
      authService.register('test@example.com', '12345678', 'Sam', 20),
  ...
  it('登入時找不到帳號', async () => {
    (userRepo.findOne as jest.Mock).mockResolvedValue(null);
    await expect(
      authService.login('test@example.com', '12345678'),
    ).rejects.toThrow();
  });
